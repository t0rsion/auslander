# Migration to the certified algebra API

Every breaking change appears in the order users most often encounter it. The
`after` snippets target the certified algebra API captured here. The current
API has moved `MonomialPresentation` and the monomial constructors into the
`monomial` module, where they analyze only. `Algebra::from_monomial` is gone,
so a monomial ideal reaches `Algebra` as an ordinary `Presentation`. Use the
API docs for current signatures.
