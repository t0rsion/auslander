# v0.6 design: checked complexes

This document is the binding specification for v0.6. Implementation follows
it exactly. A deviation needs a documented reason and a design update in the
same commit.

v0.3 certified algebra construction. v0.4 made homological answers carry
checked witnesses. v0.5 made a complete enumeration depend on a closure
certificate. v0.6 adds the checked complex needed for higher homological
claims.
...
- A net-negative production code count against v0.5.
...
value, not exception text. `test_v06_long_calls_release_gil` runs a Python
...
against QPA's terminal zero differential. The schema v8 reader requires every
...
A live GAP and QPA run must produce the committed schema v8 data. One uniquely
designated projective-dimension-one candidate over `F_5` is the control. The
...
- Every manifest declares version 0.6.0.
...
copy the removed v0.5 tilting module. Further deletion comes from shared
...
projective-dimension-one schema v8 control. The schema v8 generator checks the
...
`crates/auslander-py/pyproject.toml`, and `Cargo.lock` declare 0.6.0. The
release workflow still derives its version from the tag and must reject any
manifest mismatch.
