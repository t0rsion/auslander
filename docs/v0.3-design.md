...
- Termination and interreduction: run to a fixed point, then interreduce
  to the unique reduced Gröbner basis (monic; no leading word divides a
...
- `basis`: the reduced Gröbner basis.
...
8. `finiteness`: decide finiteness by acyclicity of the verifier's own
   ...
   verify the witness in full: the cycle is nonempty, `prefix·cycle²` is
...
pub struct Algebra {
    quiver: Quiver,
    field: PrimeField,
    relations: Vec<Relation>,      // the reduced Gröbner basis
...
- `Algebra::new(presentation, limits) -> Result<Arc<Algebra>, AlgebraBuildError>`
  runs completion, serializes the certificate, verifies the BYTES, and
  builds tables from the verified basis using the VERIFIER's division
  routine.
- `Algebra::from_verified(v: VerifiedCompletion) -> Arc<Algebra>` is the
  reverify workflow entry (dump bytes, reload, verify, rebuild). It keeps
  the default limits: certificate bytes never carry or select downstream
  budgets. `Algebra::from_verified_with_limits(v, limits)` preserves
  budgets across a reload.
...
- `Algebra::from_monomial(field, &MonomialPresentation, limits)` routes
  monomial input through the SAME pipeline (each forbidden word is a
  one-term relation). `monomial_completion_limits` derives limits that
...
- `dim`, `quiver`, `field` (NEW), `basis`, `path_index`,
...
New: `commutative_square(field)` (relation `ab - cd`) and
`preprojective(dynkin, field)` if time allows (acceptance fixtures may
inline their presentations instead).
...
- `module.rs`: `Module::new(algebra: Arc<Algebra>, dims, maps)` — the
  ...
  Validation: every relation (the reduced Gröbner basis) acts as zero:
...
- `hom.rs`, `linalg.rs`: linear algebra over module matrices — signature
  churn only (field now from algebra).
- `radical.rs`: module-level radical (`M·J` via arrow images) unchanged;
  any algebra-level series via word LENGTH must switch to the row-space
  iteration of section 6.
...
- `opposite.rs`: the opposite algebra reverses the quiver and every
  ...
  deglex with the reversed arrow order — do not rely on it; verify).
  `ElementMatrix` entries are already coefficient vectors; products now
  route through `nf_word`/tables.
...
- Acceptance matrix: duality, `ElementMatrix`, `EndoAlgebra`,
  ...
  radical/socle, `decompose`/`krull_schmidt`, `is_isomorphic`, both
  `tau` routes — each exercised over the commutative square and at least
...
Per AGENTS.md: STE-leaning docs, no separator comments, humanizer rules,
comments only for constraints the code cannot express. Commit messages:
