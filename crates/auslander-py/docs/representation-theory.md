...
[Package README](../README.md) | [Python workbench](workbench.md) | [Checkpoints and theorem artifacts](checkpoints.md) | [Compute requests](compute.md) | [Algebra and homological computations](algebra.md)

## Isomorphism and decomposition
...
## The AR translate and injectives
...
## Tau-rigidity and support tau-tilting
...
## Mutation and graph enumeration
...
An example over linearly oriented A_2:
...
Threads and interrupts: computations that can run long release the GIL.
Replacement, derived Hom, automatic transport, bounded derived discovery, and
artifact verification accept `ComputationControl`. Cancellation returns a
typed unfinished value at the next checked boundary. Older computations still
observe Ctrl-C only after the call returns. Two threads may build the same
algebra over one field at the same time. One cached result is kept, and modules
from both results still interact.
