...
use auslander::algebra::{
    MonomialAlgebra, cyclic_nakayama, dual_numbers, kronecker, linear_an, radical_square_zero_cycle,
};
...
fn opposite_involution_and_dimension_on_fixtures() {
    for algebra in fixtures() {
        ...
        let double = opposite(op.opposite());
        assert_eq!(double.opposite().quiver(), algebra.quiver());
        assert_eq!(double.opposite().forbidden(), algebra.forbidden());
    ...
    for algebra in fixtures() {
        ...
        for field in fields() {
            ...
            for v in 0..n {
                ...
                let m = Module::injective(&algebra, field, (v + 1) % n);
                let l = Module::simple(&algebra, field, v);
    ...
    for algebra in fixtures() {
        for field in fields() {
            for v in 0..algebra.quiver().num_vertices() {
                let nu = nu_of_presentation_map(&identity_element_matrix(&algebra, field, v));
...
fn nakayama_kernels_recover_hand_derived_ar_translates() {
    for field in fields() {
        let a3 = linear_an(3);
        assert_eq!(tau_dims(&Module::simple(&a3, field, 0)), vec![0, 1, 0]);
        assert_eq!(tau_dims(&Module::simple(&a3, field, 1)), vec![0, 0, 1]);
        for v in 0..3 {
            assert_eq!(tau_dims(&Module::projective(&a3, field, v)), vec![0, 0, 0]);
        }
        let kron = kronecker(2);
        assert_eq!(tau_dims(&Module::simple(&kron, field, 0)), vec![3, 2]);
        let dn = dual_numbers();
        assert_eq!(tau_dims(&Module::simple(&dn, field, 0)), vec![1]);
        for algebra in [
            radical_square_zero_cycle(3),
            cyclic_nakayama(&[3, 3, 3]).unwrap(),
        ] {
            for i in 0..3u32 {
                ...
                assert_eq!(
                    tau_dims(&Module::simple(&algebra, field, i)),
                    expected,
                    "τ S_{i} over F_{}",
                    field.modulus()
                );
...
fn transpose_route_agrees_with_the_nakayama_kernel_route() {
    for algebra in [linear_an(3), dual_numbers(), radical_square_zero_cycle(3)] {
        let op = opposite(&algebra);
        for field in fields() {
            for v in 0..algebra.quiver().num_vertices() {
