...
//! contiguous factor. Construction certifies the basis finite or fails, so dimension,
//! Cartan data, and multiplication are exact; nothing in this module truncates.
...
/// idempotents). Basis order, fixed: `basis[v]` is the trivial path `e_v` for
/// `v < num_vertices`; the rest sorted by length, then source vertex, then
/// lexicographic arrow word.
///
/// ```
...
impl MonomialAlgebra {
    ...
    /// language is infinite, decided exactly by cycle detection on the forbidden-prefix
    /// automaton.
...
/// the generated ideal is unchanged. Result sorted by length, then lexicographically.
...
/// transition is absent exactly when appending the arrow completes a forbidden word
/// (or leaves the vertex's arrows). Minimality of the forbidden set guarantees no
/// prefix state contains a forbidden factor, so every state is reachable from a start
...
mod tests {
    ...
    fn linear_kupisch_3_3_2_rejected() {
        // Regression: the old library silently built garbage from this series.
        assert!(matches!(
